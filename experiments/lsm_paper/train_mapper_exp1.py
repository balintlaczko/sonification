...
def main():
    ...
    parser.add_argument('--ckpt_name', type=str, default='imv_new_v11_testrefactor', help='checkpoint name')
    ...
    def cb_get_target_domain_latents():
        ...
        norm_params, freqs, amps = model.out_model.sample_sine_params(model.batch_size)
        x = model.out_model.input_synth(freqs).detach() * amps.unsqueeze(1)
        # select a random slice of model.n_samples
        start_idx = torch.randint(0, model.out_model.sr - model.out_model.n_samples, (1,))
        x = x[:, start_idx:start_idx + model.out_model.n_samples]
        # add random phase flip
        phase_flip = torch.rand(model.batch_size, 1, device=model.device)
        phase_flip = torch.where(phase_flip > 0.5, 1, -1)
        x = x * phase_flip
        in_wf_slice = x.unsqueeze(1)
        # forward pass
        # get mel spectrogram
        in_spec = model.out_model.mel_spectrogram(in_wf_slice.detach())
        # convert to dB
        in_spec = model.out_model.amplitude_to_db(in_spec)
