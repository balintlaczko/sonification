    ...
    def on_validation_epoch_end(self) -> None:
        ...
        if self.args.dynamic_kld > 0:
            if self.auto_dkld_scale > 0: # in auto scale mode we scale in both directions
                ...
                self.kld_weight_dynamic = torch.clip(ema(last_val, new_val, alpha=self.ema_alpha), 0.01, 100)
        ...
        if self.args.dynamic_tc > 0:
            if self.auto_dtc_scale > 0:
                ...
                self.tc_weight_dynamic = torch.clip(ema(last_val, new_val, alpha=self.ema_alpha), 0.01, 100)
